    ...
    def fit(
        ...
        for epoch in train_msg:
            ...
            for batch in train_loader:
                ...
                #### Physics based loss ####
                # Temperature constraint: Ts > Tr                
                # slack1 = y_hat[:, washout:, 1] - y_hat[:, washout:, 0] # 20x170 Tr-Ts
                # sum1 = 0
                # for i in range(0, slack1.shape[0]):
                #     for j in range(0, slack1.shape[1]):
                #         slack1ij = max(slack1[i,j], 0) #ReLU
                #         sum1 = sum1 + slack1ij**2 # for a matrix: Frobenius norm = sqrt(sum_i sum_j |a_ij|^2)
                # N = slack1.shape[0] * slack1.shape[1] # 20*170
                # constraint_temp1 = sum1/N
                # w = 0.002
                # loss = criterion(y_hat[:, washout:, :], y_batch[:, washout:, :]) + w * constraint_temp1 + w * constraint_temp2 + w * constraint_temp3 + w * constraint_temp4 + w * constraint_temp5
