...
import ssnet
from ssnet import Physics_based_general
...
# LOADS 1 4 2 3 5 (Ts, Tr, mfr) + Trit mfr_rit
U_train = ensure2D(np.genfromtxt('Datasets/Aroma/Train_Aroma_7gg_RealP_highconsumption.csv', delimiter=',',  usecols = (0,1,4,2,3,5)))
Y_train = ensure2D(np.genfromtxt('Datasets/Aroma/Train_Aroma_7gg_RealP_highconsumption.csv', delimiter=',', usecols = (6,11,16,9,14,19,7,12,17,8,13,18,10,15,20,21,22)))
U_val = ensure2D(np.genfromtxt('Datasets/Aroma/Val_Aroma_7gg_RealP_highconsumption.csv', delimiter=',',  usecols = (0,1,4,2,3,5)))
Y_val = ensure2D(np.genfromtxt('Datasets/Aroma/Val_Aroma_7gg_RealP_highconsumption.csv', delimiter=',',  usecols = (6,11,16,9,14,19,7,12,17,8,13,18,10,15,20,21,22)))
U_test = ensure2D(np.genfromtxt('Datasets/Aroma/Test_Aroma_7gg_RealP_highconsumption.csv', delimiter=',',  usecols = (0,1,4,2,3,5)))
Y_test = ensure2D(np.genfromtxt('Datasets/Aroma/Test_Aroma_7gg_RealP_highconsumption.csv', delimiter=',',  usecols = (6,11,16,9,14,19,7,12,17,8,13,18,10,15,20,21,22)))
...
def train_rnn_model(train_batch_size: int, Ts: int = 200, Ns: int = 200, lr: float = 1e-3):
    ...
    #net = Physics_based_Aroma.PhysicsNet(batch_first=True, input_scaler=input_scaler, output_scaler=output_scaler)
    net = Physics_based_general.PhysicsNet(batch_first=True, input_scaler=input_scaler, output_scaler=output_scaler)
    
    #net.init_optimizer(torch.optim.RMSprop, lr=lr, alpha=0.9, momentum=0.1, centered=True)
    net.init_optimizer(torch.optim.Adam, lr=lr) # optimization algorithm
...
training_results, descr_str = train_rnn_model(Ts=200, Ns=200, train_batch_size=20, lr=3e-3)
